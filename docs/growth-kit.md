...
### 3b.0 先修：频道上有两处线上违规 ⚠️

`t.me/Aliboby88` 已经是一个**频道**（不是个人号，t.me 上的动作是 "Preview channel"，因此**不能私聊**），只有 2 个订阅者。`t.me/s/Aliboby88` 实测可爬（HTTP 200、36KB、置顶帖正文可读），所以下面两处都处于**可被搜索引擎索引**的状态。

两处都只能运营方在 Telegram 客户端内修改。

#### (a) 置顶帖泄露 §3.10 内部专用数据 —— 更严重

置顶帖（1009 字符）整体是照现行 canon 写的，但其中两行发布了 §3.3 / §3.4 / §3.10 明令不得对外的内容：

要删的是**那两条 `•` 行里 `سرخس/لطف‌آباد ` 开头的那一段**（直达行与换装行各一处，形如 `سرخس/لطف‌آباد ⟨两位数–两位数⟩、`）。同一行里 `مشهد` 与 `تهران` 的天数是已公开口径，保留。

这两段正是 `AGENTS.md` §3.3 所列的边境海关内部时效，`لطف‌آباد` 则是 §3.4 规定仅为内部事实的地名——数字本体见 §3.10，此处不再复述。

另有两处非违规但需要修的缺陷：原帖没有说明**时效自乌恰口岸起算**（买家会误读成从origin仓库起算，这是 §3.3 的核心限定），以及 `حد بار … (جزئیات با نوع کامیون)` 的括号暗示了载量按车型有差异——而"是否按直达/换装区分"是 §3.9 的未决项，不该在对外文案里暗示答案。

**整帖替换文本**（1501 字符，Telegram 帖子上限 4096；已过 canon 正反向双向校验）：

```
chinairantrucks — حمل جاده‌ای چین به ایران

دو خط در فروش: ووچیا و خورگوس.
تحویل انبار/ترمینال به انبار مقصد — نه درب‌تا‌درب، نه DDP، نه ترخیص گمرکی ایران.

خط ووچیا (سین‌کیانگ → قرقیزستان → ازبکستان → ترکمنستان → ایران):
• زمان‌ها از مرز ووچیا شمرده می‌شود، نه از انبار مبدأ. مسیر داخلی چین تا ووچیا جداگانه است — برآوردی، معمولاً ظرف ۷ روز.
• مستقیم (فقط یک تعویض در بخارا): معمولاً مشهد ۱۸–۲۱، تهران ۲۱–۲۳ روز
• تعویض بار (اوش + بخارا و بیشتر): معمولاً مشهد ۲۴–۲۸، تهران ۲۶–۳۰ روز
• ورود به ایران از سرخس. مقصدهای اعلامی: گمرک مشهد و تهران (گمرک غرب تهران، شهریار، آپرین).
• مالیات ترانزیت ۰٫۴٪ قرقیزستان روی ارزش فاکتور — جدا از کرایه؛ در پیش‌فاکتور جدا می‌آید

خط خورگوس (سین‌کیانگ → قزاقستان → ازبکستان → ترکمنستان → ایران):
• بدون ۰٫۴٪
• زمان حمل این خط فعلاً اعلام نمی‌شود — استعلامی

TIR: پلمپ مبدأ + یک کارنه؛ باز کردن و تعویض کمتر است، نه صفر. زمان و کرایه را عوض نمی‌کند. کارنه را حمل‌کننده تهیه می‌کند، نه صاحب کالا.

حد بار: تا ۹۶ مترمکعب و ۲۵ تن در هر دستگاه. مازاد جداگانه قیمت می‌خورد.
خرده‌بار (LCL): از ۱ مترمکعب یا ۱۰۰ کیلوگرم.
فقط کالای عمومی؛ خطرناک / شیمیایی / برند نیاز به بررسی مدارک (باتری لیتیومی با MSDS کامل ۱۶ بخشی و مدارک بسته‌بندی UN).

در کرایه نیست: ترخیص مقصد، بیمه، بازرسی، مهاربندی، جرثقیل، و عوارض ۰٫۴٪ خط ووچیا.

قیمت در کانال اعلام نمی‌شود. مشخصات بار (نام کالا، کد HS، حجم/وزن، مقصد، ووچیا یا خورگوس) بفرستید تا پیش‌فاکتور رسمی بفرستیم. نرخ‌ها هفتگی بازبینی می‌شود و هر پیش‌فاکتور تاریخ اعتبار دارد.

📄 https://chinairantrucks.com
✉️ sales@chinairantrucks.com
```

相对原帖的改动仅四项：删掉两行边境海关内部数字与 `لطف‌آباد`、补入"自乌恰起算 + 国内段 ≤7 天另计"、补入 `سرخس` 作为入境口岸与已公开的销售目的地、载量改为"每台"并补 LCL 起运量与不含项。其余措辞保留原作者的写法。

#### (b) 频道简介仍是作废口径

简介正在公开发布**已废止的旧 canon**：
...
**简介替换文本**（Telegram 频道简介上限 255 字符，以下约 200）：
