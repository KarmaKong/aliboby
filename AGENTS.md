...
- **`tools/prep-photo.sh` + §7 photo rule extended**（2026-09-15）— the operator drops real operation photos into `~/Documents/货照片` (dated batches plus category folders) and will keep adding there. The script does the §7 mechanics (resize to 1600, strip EXIF/GPS, q80 progressive, print the `<img>` tag); content clearance stays manual.
 ...
 - **Open, not done: the existing `img/` set predates this rule.** `ftl-crates-rear-tm-plate.jpg` — already live, and used as the hero on the new log pages — has a third-party logo legible on cartons in the background. The other 18 published photos have not been re-checked against the new clause. Worth an audit pass; not urgent enough to block, since these have been public for a while already.
