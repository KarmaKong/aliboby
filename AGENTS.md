...
- **发车日志 `departures` — 三语证据型内容 + 生成器**（2026-09-15）— the first content type on this site that is *evidence* rather than *explanation*, and the first one designed to be maintained by the operator rather than by an agent.
 ...
 - **Seeded with 3 real loads** (2026-08-31 / 09-02 / 09-09, all Yiwu → Wuqia line → Tehran, direct). Only the 08-31 row is documented end-to-end by an operator 出货记录; the cargo type on the other two was **inferred from the photographs** (a loaded car carrier on 09-02, mixed carton consolidation on 09-09) and is flagged for operator confirmation. Do not treat inferred cargo types as canon.
