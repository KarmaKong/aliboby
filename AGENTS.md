...
- **Stack:** hand-written static HTML + one CSS file. No framework, no build step, no JS app.
- **Host:** GitHub Pages, "deploy from branch", source = `main` / root. `CNAME` = `chinairantrucks.com` (apex, no `www` — `www` 301s to apex). Jekyll is left **on** (no `.nojekyll`) so `_config.yml` `exclude` can keep agent manuals and build inputs off the live site: `AGENTS.md`, `CLAUDE.md`, `docs/`, `tools/`, `data/`, `trucks/`, `brand/telegram-qr.png`. HTML without front matter is copied through unchanged. Article templates (`articles/_post.html` and `zh/articles/_post.html`) are unpublished because they start with `_` — that is intended. Do not add `.nojekyll`; that would publish the manuals again.
...
- **Carnet side (operator-asserted, 2026-09-14):** this line runs on a **China-side TIR carnet held by us as the TIR operator** — consistent with the long-standing published answer "the carrier holds the carnet, the shipper does not obtain it". It is therefore **not** gated by the availability or price of **Iranian-issued** carnets, which Iranian industry bodies have been reporting as short and more expensive. Scope this claim tightly: it covers **carnet supply only**. It does **not** mean immunity from border congestion, driver-visa problems, or any other shared road condition, and per the line above it changes neither transit time nor rate. Never turn this into a dig at Iranian carriers — the audience includes Iranian brokers.
...
- Any freight rate.
- **The `ISACO` / `isacogroup` entity name and the `exim@isacogroup.com` address** — a **brand rule, not a secret** (operator-asserted, 2026-09-07, recovered from the closed PR #55). Until PR #39 this was the site's own published contact address: 285 occurrences across 71 files — `mailto:` links, buttons, footers, Organization JSON-LD `email` and `contactPoint`, `llms.txt`, and both `_post.html` templates — on a public repo and a live public site, so it is in the git history from the first commit onward and in PR #39's title. **Do not try to purge it**; that would mean rewriting the entire history and would still not remove it from GitHub metadata, mirrors, or search-engine caches. What the rule actually requires is forward-looking: the public identity is **chinairantrucks** only, and the sole publishable address is `sales@chinairantrucks.com`. Never reintroduce the old entity name or address onto a public surface — page copy, JSON-LD, `llms.txt`, `sitemap.xml`, or social / WhatsApp / Telegram text. The real failure mode is copying content out of operator-supplied source documents, which still carry the old address, so check for it whenever you do.
...
- `img/` — real operation photos. In use: `yard-dsab-loading.jpg` (1600×1067), `ftl-crates-rear-tm-plate.jpg` (1600×931), `tractor.jpg` / `warehouse.jpg` / `crate-marks.jpg` / `forklift.jpg` (1600×1000), `ftl-bagged-cargo.jpg` (1600×552), `pallets-strapped.jpg` (1279×1406), `curtainsider-resin-pallets.jpg` (1600×1200, hero of the three departure-log pages). `img/log/` holds the dated photos the departure log references: `2026-08-31-curtainsider-crated-machinery.jpg` (1600×1200), `2026-09-09-loading-cartons.jpg` (1600×900). Compress to ~q80 progressive JPEG, strip EXIF, **no readable licence plates, no faces, no watermarks, no sensitive documents** — blur or crop first. `tools/prep-photo.sh SRC DEST` does the resize / strip / q80 mechanically and prints the `<img>` tag; it does **not** judge content, so the clearance below is still by eye.
 - **No identifiable third-party cargo brand, and no readable consignee or company name** (added 2026-09-15 after a triage of the operator's photo folder found appliance brand names printed across cartons and a company name legible on a crate label, in shots that had been marked as ready to publish). This is **customer confidentiality, not housekeeping**: the audience is Iranian importers, and a photo that shows whose goods are moving on this corridor exposes the customer, not us. Unbranded cartons are fine; the operator's own yard equipment (forklift make and so on) is fine. Shipping-mark reference codes with no company name in them are a judgement call — prefer to crop.
 - Source photos arrive in `~/Documents/货照片` (dated batches plus `仓库` / `装车` / `装柜` / `货物特写`). WeChat has already stripped EXIF from those, so the dates come from the folder and filename convention, not from metadata — fine for a log, but do not describe them as metadata-verified.
...
- Modify `js/count.js` (GoatCounter's own file), `CNAME`, `_config.yml`, the IndexNow key file, or `google869b982389a5864a.html`. Do not add `.nojekyll`.
...
- **发车日志 `departures` — 三语证据型内容 + 生成器**（2026-09-15）— the first content type on this site that is *evidence* rather than *explanation*, and the first one designed to be maintained by the operator rather than by an agent.
 - **How it works.** `data/departures.tsv` is the only file a human touches: one row per load, all enum fields (`hub` / `cargo` / `line` / `dest` / `mode` / `status`) plus a date, a day count and an optional photo. `tools/build-departures.py` turns those rows into all three languages and rewrites only the region between the `DEPARTURES:BEGIN` / `DEPARTURES:END` markers on the three pages, per the §12 exact-substring rule. **No prose is ever hand-written per entry**, which is the whole point — the operator physically cannot type a banned phrase, and does not have to hand-set Persian digits.
 - **Canon guards are in the generator, not in review.** It refuses to build if `dest` is anything but `tehran` / `mashhad` (so Sarakhs and Lotfabad cannot leak in from §3.10), if a `khorgos` row carries a day count (§3.3 has no published figure for that line), or if fewer than 3 entries exist. A day count outside the published range is a warning, not a hard error — real single loads do vary — and is overridden by appending `!force` to the row, which forces a deliberate decision instead of a silent publish. All four were tested.
 - It also prints the Persian Telegram post for the newest entry, which is what §3b.4 of the growth kit asks for. That closes the loop the growth kit left open: the channel now has a content type that is generated from the same canon-checked source as the site.
 - Wired in per §5: way-row on all three indexes, 3 sitemap URLs (now 94), `llms.txt`, and two entity-named inbound links per language from `22-days` and `transit-time` so the page is not born an orphan. hreflang verified symmetric with `x-default` → Persian.
 - **Seeded with 3 real loads** (2026-08-31 / 09-02 / 09-09, all Yiwu → Wuqia line → Tehran, direct — routing fields operator-supplied). The 08-31 cargo type comes from an operator 出货记录. The other two were first **inferred from the photographs** — a loaded car carrier on 09-02, mixed carton consolidation on 09-09 — and then **confirmed by the operator on 2026-09-15** as `vehicles` and `general`. Keep that order in mind for future rows: infer to draft, but do not publish a cargo type until the operator confirms it, since the log's whole value is that it is not guesswork.
 - The **09-02 row has no photo on purpose.** It is the vehicle-carrier load, and the photo set contains no publishable vehicle-carrier shot — every candidate carries a readable plate. Leaving the slot empty was preferred over illustrating a "complete vehicles" entry with a carton photo, which would misdescribe the load.
 - **Two bugs were caught by verification, both in the generator, before anything shipped.** It hardcoded `height="1200"` on log photos, which is wrong for any 16:9 source — it now reads real dimensions out of the JPEG header. And it emitted `../../img/log/…` for all three languages, which is correct only for `fa`; `zh` and `en` sit one level deeper and would have shipped two broken images each. That second one slipped past the standard §12 link check because **that check only looks at `href`, not `src`** — when reusing it, check both, and compare each `<img>` width/height against the real file.
- **`tools/prep-photo.sh` + §7 photo rule extended**（2026-09-15）— the operator drops real operation photos into `~/Documents/货照片` (dated batches plus category folders) and will keep adding there. The script does the §7 mechanics (resize to 1600, strip EXIF/GPS, q80 progressive, print the `<img>` tag); content clearance stays manual.
 - **Why §7 grew a clause.** A triage of that folder found that 3 of the 4 photos the operator had marked as ready for the website could not be published: one had a forklift driver's face, one had a readable licence plate, and — the one the old §7 did not cover — one showed a third-party appliance brand printed across the cartons plus a legible company name on a crate label. For an Iran-facing line that is **customer exposure**, which is why the new clause is framed as confidentiality rather than housekeeping.
 - **Full triage of the 143 photos: 30 publishable as-is, 41 usable only after a blur or crop, 72 not usable.** The dominant reject reason by a wide margin is the new clause — **47 of the 72 are third-party cargo branding** (appliance and polymer brands printed across the packaging), which is what turned an "add a note" into a real rule. The practical lesson for future shoots, worth passing to the operator: **frame the trailer, the lashing and the yard, not the carton faces** — that one change converts most of the loss. Also note the set contains **no clean border or road scene at all**: every road candidate carries a Central Asian plate, so that theme needs either an edit or a new shoot.
 - **Do not trust an automated photo triage as the final gate.** The pass that produced those numbers marked a night shot of a Renault tractor as publishable when it has a fully readable Kazakh plate (`974 AVR 08`) across the bumper. Everything actually published was re-checked by eye at full resolution first, including zooming into label areas; keep doing that.
 - **Open, not done: the existing `img/` set predates this rule.** `ftl-crates-rear-tm-plate.jpg` is already live and has a third-party logo legible on cartons in the background. It was the first hero of the departure-log pages and has been swapped out for `curtainsider-resin-pallets.jpg`, but it is **still in use on other pages**, and the other 18 published photos have not been re-checked. Worth an audit pass; not urgent enough to block, since these have been public for a while already.
...
- **v3 "Swiss 速度感" redesign** — spec at `Claude-Code-速度感改版-v3.md` (design package, off-repo, on the operator's machine, not checked into this repo). Execution order is staged per the spec's §11:
 ...
 - **Stage 2 follow-up (2026-09-14):** the three `body.fa` hero overrides were removed, so all three homepages now share one wash and one crop. They had assumed the Persian copy sits on the photo's bright right-hand side, but `body.fa .hero-copy { margin-inline-start: auto }` actually places it on the **left** (measured at 11.8%–51.8% of the hero width, the same side as zh/en). The `to left` wash was therefore covering the truck and sunset instead of the text, which is what made the Persian truck look dim. The remaining `body.fa` hero rules are typographic only (copy-box width, h1 size) and must stay. If you ever move the Persian copy back to the right, the wash direction has to move with it.
...
- **`tir-carnet-shortage` article** (2026-09-14) — new 三语 article answering the question Iranian importers are actually asking right now: Iranian road-transport associations are reporting a TIR carnet shortage and higher issuing cost, so does cargo already moving from China stop? Built on the §3.8 carnet-side fact plus existing canon; no third-party figures (carnet price percentages, border-queue counts) are quoted, because those are volatile and unverified. Keep it that way. The angle is deliberately explanatory, not competitive — see the tone warning in §3.8.
- **Two latent bugs fixed in `articles/_post.html`** (2026-09-14) — the fa template pointed at `../js/count.js`, `../js/lang.js` and `../favicon*` (one level short), so every article duplicated from it would have shipped with broken analytics, a broken language switcher and broken icons. The zh template and all real fa articles already used `../../`. Both now match. No published page was affected.
- **zh / en internal-link rebuild** (2026-09-14) — the same fix the fa pass got, now applied to the other two languages, which had the identical defect: `china-to-iran-tir-trucking` was linked 19× with the single anchor `干线总览` / `Route overview`, `quote` 10–13× with one anchor, and four (zh) / five (en) pages had zero inbound links (`uzbekistan`, `hormuz-insurance`, `lcl`, `cku-railway`, plus `auto-parts` in en). Also fixed a genuine mismatch carried in all three languages: five anchors pointing at `wuqia-khorgos` read "Khorgos or Irkeshtam", but that page is *Wuqia* or Khorgos — `xinjiang-ports` is the Khorgos/Irkeshtam page. Anchors are now entity-named and rotated (max repeat 6, was 19), every page has ≥2 inbound links, zero dead internal links. All 26+26 pages verified with the §12 BeautifulSoup-style diff: zero drift outside the `post-links` block.
- **`docs/growth-kit.md` — Telegram/Instagram section + canon fixes** (2026-09-14) — added §3b (Telegram, Persian) and §3c (Instagram), and fixed three canon violations that were sitting in copy intended for publication on LinkedIn and B2B directories: the EN long bio claimed **"No subcontracting layer"** (banned by §3.8) and put dispatch "at Khorgos" instead of the exit gateway; LinkedIn Post 6 said the trailer **"is sealed once and doesn't get opened again until Tehran"** (banned by §3.2); the sitemap URL count was stale at 75 (now 91). Check this file for canon drift whenever §3 changes — it is off-site copy, so nothing on the site catches it.
 - **Telegram channel description — fixed by the operator 2026-09-15, verified.** `t.me/Aliboby88` had been advertising the superseded **`۱۴–۱۸ روز`** plus a `مستقیم … زیر پلمپ` framing; it is referenced from the three homepages' quote strip and the Organization `sameAs`, so it was the first thing an Iranian buyer saw after leaving the site. The live description now reads `۲۱ تا ۲۳ روز` with the `فعلاً` hedge, counts from `از مرز ووچیا`, states `یک تعویض بار در بخارا` outright, and lists only Tehran West Customs and Mashhad. That is canon-clean. Two wording notes remain, both minor: it says `کانتینری و خرده‌بار` where the site's own Persian entity terms are **`دربست`** (37 uses on-site; the fa `ftl-vs-ltl` title is `دربست یا کنسول`) and **`کنسول`** — and `کانتینری` additionally risks reading as sea-container service, which this line does not sell. Keep off-site copy on the site's terms so the entity signal is not split, the same reason `خورگوس` was normalized.
 - **Still open, operator-side: one channel post names `لطف‌آباد`.** A token scan of the 8 crawlable posts on `t.me/s/Aliboby88` found Lotfabad in the route-overview post, which §3.10 forbids on any public surface. The more serious leak is already gone — the internal border-customs day figures (`۱۵ تا ۱۸` / `۲۱ تا ۲۴`) no longer appear anywhere on the channel, and neither do `۱۴ تا ۱۸`, `بدون تخلیه` or `تضمین`. `سرخس` appears in two posts and is fine; §3.4 publishes Sarakhs as the entry/transit gateway. **Only Lotfabad has to come out.**
 - When scanning the channel, note that a naive prohibition list will flag `سرخس` as internal. It is not — only Lotfabad and the border-customs day table are (§3.10). Getting this backwards wastes the operator's time on a non-issue.
 - Verified for the plan, not assumed: `t.me/s/<channel>` is server-rendered and crawlable (146 KB HTML, 20 readable message bodies on a test channel) and `t.me` serves no robots.txt, so a public channel is a genuinely rankable Persian surface. Instagram is the opposite — Meta blocks crawlers, so it is a trust/conversion channel only and is scoped as lower priority.
 - **Open decision, do not act on it unilaterally:** in Iran Telegram penetration is ~59% vs WhatsApp ~41% (both filtered, both used via VPN), which cuts against §2's site-wide WhatsApp-primary / Telegram-secondary CTA hierarchy **on the Persian pages specifically**. B2B importer habits may differ from the general population, so this is flagged in §3b.2 as an operator decision, not a change.
- **Persian entity + internal-link pass** (2026-09-14) — two fa-only fixes aimed at Iranian search visibility.
 - **Khorgos spelling normalized to `خورگوس`** (159 replacements, 23 files, including 14 `<title>`/`<meta description>`). The site had been writing `هورگوس` 157 times against `خورگوس` 25 — and 17 of those 25 were all inside `xinjiang-ports`, so the site contradicted itself. `خورگوس` is what §3.1 already specifies, what the Tehran Chamber of Commerce uses (`منطقه ویژه اقتصادی خورگوس`), and what the Iranian competitor currently ranking for the core query uses. The old spelling is kept alive in exactly one place — a parenthetical on first mention in `wuqia-khorgos` — so the variant query is still covered without splitting the entity signal. **Do not reintroduce `هورگوس` as the primary spelling.**
 - **fa `post-links` graph rebuilt.** Before: five pages (`cku-railway`, `hormuz-insurance`, `lcl`, `uzbekistan`, `wuqia-irkeshtam`) had zero inbound links from any article and were reachable only from the index; the `22-days` pillar had one. Worse, all 20 internal links to the main pillar used the identical anchor `مرجع مسیر` ("route reference"), which names no entity and matches no query, and `quote` had 10× `نحوهٔ استعلام`. Anchors are now entity-named (`۲۱ تا ۲۳ روز از ووچیا تا تهران`, `کنسول از ۱ متر مکعب به گمرک تهران`, `ووچیا یا خورگوس و عوارض ۰٫۴٪`) and rotated, max repeat 5. Zero orphans, minimum inbound 2. **zh and en still have the flat single-anchor structure — same fix is unapplied there.**
 - Verified with the §12 pattern below plus a dead-link check: 28 fa files, zero content drift outside the `post-links` blocks and the spelling change.
